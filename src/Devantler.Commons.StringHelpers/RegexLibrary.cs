...
public static class RegexLibrary
{
    ...
    /// </summary>
    public static Regex CamelCaseWithDigitsRegex { get; set; } = new Regex("^[a-z][a-z0-9]*(([A-Z]{1,3}[a-z0-9]+)*[A-Z]{0,3}|([a-z0-9]+[A-Z]{1,3})*|[A-Z]{1,3})$");

    /// <summary>
    /// Regex that matches PascalCase with digits and up to three consecutive capital letters.
    /// </summary>
    public static Regex PascalCaseWithDigitsRegex { get; set; } = new Regex("^[A-Z](([A-Z]{1,2}[a-z0-9]+)+([A-Z]{1,3}[a-z0-9]+)*[A-Z]{0,3}|([a-z0-9]+[A-Z]{0,3})*|[A-Z]{1,2})$");
